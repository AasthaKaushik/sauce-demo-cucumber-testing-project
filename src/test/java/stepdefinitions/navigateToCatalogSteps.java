...
public class navigateToCatalogSteps {
	...
	public void i_click_the_add_to_cart_button() {
		PageFactory.getCatalogPage().navigateToProductPage();
		PageFactory.getCatalogPage().clickAddToCartButton();
	...
	public void i_click_on_a_product_marked_sold_out() {
		PageFactory.getCatalogPage().clickonBrownShadesProduct();
	}

	@Then("there should be SOLDOUT button must be disable")
	public void there_should_be_soldout_button_must_be_disable() {
		WebElement soldOutButton = PageFactory.getCatalogPage().getSoldOutDisable();
